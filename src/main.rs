...
impl<'a> Handler for Server<'a> {
	fn on_message(&mut self, msg: Message) -> ws::Result<()> {
		let edit: Edit = serde_json::from_str(msg.as_text()?).map_err(|err|
			ws::Error::new(ws::ErrorKind::Custom(Box::new(err)), "invalid json"))?;
	...
	fn on_request(&mut self, req: &Request) -> ws::Result<Response> {
		match req.resource() {
			...
			_ => Ok(Response::new(404, "Not Found", Vec::from("404 - not found"))),
		}
	}
}
...
fn main() {
	...
	listen("0.0.0.0:8080", |out| Server { editor: &editor, out }).unwrap();
}
